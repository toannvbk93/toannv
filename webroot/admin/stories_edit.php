...
if($_POST['submitform'] == "1")
{
	if($PID > 0)
	{
		$marknew = intval(cleanit($_REQUEST['marknew']));
		if($marknew == "1")
		{
			$sql = "update posts set ttime='".time()."', htime='".time()."' where PID='".mysql_real_escape_string($PID)."'";
			$conn->execute($sql);
		}
		$arr = array("USERID", "story", "tags", "source", "CID", "nsfw", "phase", "favclicks", "view", "blog", "unfavclicks", "active");
