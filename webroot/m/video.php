...
else
{
	...
	$query1 = "SELECT count(*) as total from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.pic= '' order by A.time_added desc limit $config[maximum_results]";
	$query2 = "SELECT A.*, B.username from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.pic= ''  order by A.time_added desc limit $pagingstart, $config[mobile_per_page]";
...
if ($totalvideos > 0)
{
	...
	if ($currentpage > 0)
	{
		if($currentpage > 1) 
		{
			$pagelinks.="<a href='$mobileurl/video?page=1'>$lang[64]</a>&nbsp;&nbsp;";
			$pagelinks.="...&nbsp;&nbsp;";
			$pagelinks.="<a href='$mobileurl/video?page=$theprevpage'>&laquo; $lang[65]</a>&nbsp;&nbsp;";
		...
		{
			$pagelinks.="<a href='$mobileurl/video?page=$lowercount'>$lowercount</a>&nbsp;&nbsp;";
			$lowercount++;
			$counter++;
		}
		...
		{
			$pagelinks.="<a href='$mobileurl/video?page=$uppercounter'>$uppercounter</a>&nbsp;&nbsp;";
			$uppercounter++;
		}
		
		if($currentpage < $toppage) 
		{
			$pagelinks.="<a href='$mobileurl/video?page=$thenextpage'>$lang[66] &raquo;</a>&nbsp;&nbsp;";
			STemplate::assign('tnp',$thenextpage);
			$pagelinks.="...&nbsp;&nbsp;";
			$pagelinks.="<a href='$mobileurl/video?page=$toppage'>$lang[67]</a>&nbsp;&nbsp;";
		}
	}
}
