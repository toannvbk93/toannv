...
if (isset($_SESSION['viewtype']) && isset($_SESSION['view']))
...
if ($config['trendingenabled'] == 0) {
    $query1 = "SELECT count(*) as total from posts A, members B where A.active='1' AND A.phase>'1' AND A.USERID=B.USERID order by A.htime desc limit $config[maximum_results]";
    $query2 = "SELECT A.*, B.username from posts A, members B where A.active='1' AND A.phase>'1' AND A.USERID=B.USERID order by A.htime desc limit $pagingstart, $config[items_per_page]";
} else {
    $query1 = "SELECT count(*) as total from posts A, members B where A.active='1' AND A.phase>'1' AND A.USERID=B.USERID order by A.htime desc limit $config[maximum_results]";
    $query2 = "SELECT A.*, B.username from posts A, members B where A.active='1' AND A.phase>'1' AND A.USERID=B.USERID order by A.htime desc limit $pagingstart, $config[items_per_page]";
}
...
STemplate::assign('pagetitle', 'Đang HOT');
