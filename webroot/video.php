...
if ($config['trendingenabled'] == 0)
  {
    ...
    $query2 = "SELECT A.*, B.username from posts A, members B where A.active='1' AND A.USERID=B.USERID  AND A.pic= '' order by A.time_added desc limit $pagingstart, $config[items_per_page]";
  }
else
  {
    $query1 = "SELECT count(*) as total from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.pic= '' order by A.time_added desc limit $config[maximum_results]";
...
$eurl = base64_encode("/video?page=" . $currentpage);
...
$_SESSION['location'] = "/video?page=" . $currentpage;


//TEMPLATES BEGIN
STemplate::assign('menu', 4);
STemplate::assign('pagetitle', 'Video');
if(isset($posts))
