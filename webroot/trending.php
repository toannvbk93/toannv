...
elseif (isset($_SESSION['view']) && $_SESSION['viewtype'] == "list" && $_REQUEST['view'] == "")
 {
	 $_SESSION['viewtype'] = "list";
 }
...
elseif ( isset($_SESSION['view']) && $_REQUEST['view'] != "")
 {
...
if ($config['trendingenabled'] == 0)
 {
	 ...
	 $query1 = "SELECT count(*) as total from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.view>= '100' order by A.time_added desc limit $config[maximum_results]";
	 $query2 = "SELECT A.*, B.username from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.view>= '100' order by A.time_added desc limit $pagingstart, $config[items_per_page]";
...
$eurl = base64_encode("/old?page=" . $currentpage);
...
$_SESSION['location'] = "/old?page=" . $currentpage;
//TEMPLATES BEGIN
STemplate::assign('menu', 2);
STemplate::assign('pagetitle', $lang[331]);
