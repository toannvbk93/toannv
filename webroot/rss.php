...
$get_gags = "SELECT PID, story, time_added,path,pic FROM posts WHERE active=1 ORDER BY PID DESC LIMIT 20";
...
while ($gag = mysql_fetch_array($gags)){
    ...
    echo '
       <item>
          <title>'.html_entity_decode($gag[story], ENT_COMPAT, "UTF-8").'</title>
		  ...
		  if($gag[pic] != ""){echo '
		  <enclosure type="image/jpeg" url="'.$thebaseurl.'/pdata/'.$gag[path].'/t/s-'.$gag[pic].'" length="1" />';
		  }
    echo  '
