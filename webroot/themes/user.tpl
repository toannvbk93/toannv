    ...
    <div id="content-holder">		
        <div class="profile-pad">
            ...
            <div class="profile-info" href="#" style="background:#{$p.color1}">
                ...
                <a class="friendship" href="{$baseurl}/user/{$p.USERID|stripslashes}/messages">Messages</a>
            </div>
        </div>
        <div class="main-filter with-topping">
            <ul class="content-type">            
                <li><a class="current" href="{$baseurl}/user/{$p.USERID|stripslashes}"><strong>{$lang192}</strong> ({$totalposts})</a></li>
                <li><a  href="{$baseurl}/user/{$p.USERID|stripslashes}/likes"><strong>{$lang193}</strong> ({$tl})</a></li>            
                <li><a class="" href="{$baseurl}/user/{$p.USERID|stripslashes}/messages"><strong>{$lang194}</strong> (<fb:comments-count href="{$baseurl}/user/{$p.USERID|stripslashes}/messages"></fb:comments-count>)</a></li>
            </ul>
			{if $safemode eq "1"}
        ...
        <div id="content" listPage="">
			...
			<div id="paging-buttons" class="paging-buttons">
            	{if $tpp ne ""}
                <a href="{$baseurl}/user/{$p.USERID|stripslashes}?page={$tpp}" class="previous">&laquo; {$lang166}</a>
                {else}
                <a href="#" onclick="return false;" class="previous disabled">&laquo; {$lang166}</a>
                {/if}
                {if $tnp ne ""}
                <a href="{$baseurl}/user/{$p.USERID|stripslashes}?page={$tnp}" class="older">{$lang167} &raquo;</a>
                {else}
