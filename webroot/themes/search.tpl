                   ...
                   <li class=" ">
                        <a href="{$baseurl}{$postfolder}{$posts[i].PID}/" class="jump_stop">
                            <div style="" class="thimage">
                                ...
                                {else}
                                {if $posts[i].nsfw eq "1" AND $smarty.session.FILTER ne "0"}<img src="{$baseurl}/images/nsfw_thumb.jpg" alt="{$posts[i].story|stripslashes}" />{else}{if $posts[i].isgif eq "1"}<img src="{$purl}/{$posts[i].path}/t/static-{$posts[i].pic}" alt="{$posts[i].story|stripslashes}" />{else}<img src="{$purl}/{$posts[i].path}/t/s-{$posts[i].pic}" alt="{$posts[i].story|stripslashes}" />{/if}{/if}
                                {/if}
                            </div>
                        </a>
                        <h1>{$posts[i].story|stripslashes|truncate:20:"...":true}</h1>
                        <h4><a href="{$baseurl}/user/{$posts[i].USERID|stripslashes}">{$posts[i].username|stripslashes|truncate:20:"...":true}</a></h4>
                    </li>
                </ul>
                {/section}            
            </div>
