     	...
     	{insert name=get_member_profilepicture assign=profilepicture value=var username=$posts[i].username}
            <div class="item">
                <a target="_parent" href="{$baseurl}/user/{$users[i].USERID}">
                    <img src="{$membersprofilepicurl}/thumbs/{if $users[i].profilepicture eq ""}noprofilepicture.jpg{else}{$users[i].profilepicture}{/if}?{$smarty.now}" style="width:35px; height:35px;">
                    <div class="info">
