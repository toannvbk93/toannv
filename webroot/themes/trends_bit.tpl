                    <li class=" gag-link {if $posts[i].isgif eq '1'}gifContent{/if}" data-url="{$baseurl}{$postfolder}{$posts[i].PID}" data-text="{$posts[i].story|stripslashes|truncate:20:"...":true}" gagId="{$posts[i].PID}" itemType="list" id="entry-{$posts[i].PID}">
                        <div class="content">
                            <div class="img-wrap">
                                ...
                                {else}
                                    {if $posts[i].pic ne ""}
                                        ...
                                        <div class="thumb-wrapper" id="scriptolution_gif_static{$posts[i].PID}">
                                            <a onclick="toggle('scriptolution_gif_static{$posts[i].PID}'); toggle('scriptolution_gif_ani{$posts[i].PID}');">
                                                <img src="{$purl}/{$posts[i].path}/t/static-{$posts[i].pic}" alt="{$posts[i].story|stripslashes}" />
                                                <span>GIF</span>
                                            </a>
                                        </div>
                                        <div class="gifimg" id="scriptolution_gif_ani{$posts[i].PID}" style="display:none">
                                            <a onclick="toggle('scriptolution_gif_ani{$posts[i].PID}'); toggle('scriptolution_gif_static{$posts[i].PID}');">
                                            <img src="{$purl}/{$posts[i].path}/t/{$posts[i].pic}" alt="{$posts[i].story|stripslashes}" />
                                            </a>
                                        </div>
                                        {else}
  ...
  {if $smarty.section.i.first}
      <li style="padding: 0px;">
            <div style="background-color: #EEEEEE;margin: 10px 0px 10px 0px;padding: 5px;">
                                <span style="font-size: 18px;color:#880015"><b>Like <a style="display:inline;" href="http://www.facebook.com/{$FACEBOOK_PROFILE}" target="_blank"> {$lang['367']}</a> {$lang354}</b></span>
