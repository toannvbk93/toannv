    ...
    <div id="content-holder">        
        ...
        <div id="content" listPage="hot">            
            ...
            <div id="paging-buttons" class="paging-buttons">
            	{if $tpp ne ""}
                <a href="{$baseurl}/top/{if $period eq "all"}all{elseif $period eq "month"}month{elseif $period eq "week"}week{else}new{/if}?page={$tpp}" class="previous">&laquo; {$lang166}</a>
                {else}
                <a href="#" onclick="return false;" class="previous disabled">&laquo; {$lang166}</a>
                {/if}
                {if $tnp ne ""}
                <a href="{$baseurl}/top/{if $period eq "all"}all{elseif $period eq "month"}month{elseif $period eq "week"}week{else}new{/if}?page={$tnp}" class="older">{$lang167} &raquo;</a>
                {else}
