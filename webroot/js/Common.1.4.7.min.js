var Gauvl = {};
Gauvl.Following = {};
Gauvl.Following.changeButtonState = function (a, b) {
    if ($(a).hasClass("not-follow")) {
        ...
        $("[data-followed=" + c + "]:not('#" + d + "')").each(function () {
            if ($(this) != $(a)) Gauvl.Following.changeButtonState(this, false)
        })
    }
};
Gauvl.Following.init = function () {
    var g = false;
            ...
            $.ajax({
                ...
                success: function (a) {
                    if (a == "OK") {
                        ...
                        Gauvl.Following.changeButtonState(this, true);
                        g = false
...
Gauvl.common = {};
Gauvl.common.registerVoteClick = function () {
    $(document).on("click", '.voteButton', function (a) {
        ...
        if ($(this).hasClass("voted")) {
            ...
            Gauvl.common.votePost(b, c, true, $(this))
        } else {
            $(this).parent().find(".voteButton").removeClass("voted");
            changeVoteButtonState($(this));
            Gauvl.common.votePost(b, c, false, $(this))
        }
    });
...
Gauvl.common.votePost = function (b, c, d, e) {
    var f;
...
Gauvl.indexPage.reLayout = function () {
    var a = $('#post-container');
    a.isotope('reLayout')
};
Gauvl.indexPage.setExpand = function () {
...
Gauvl.indexPage.scrollToClip = function (a) {
    setTimeout(function () {
        $('html,body').animate({
...
Gauvl.indexPage.scrollToEntry = function (a) {
    setTimeout(function () {
        $('html,body').animate({
...
Gauvl.indexPage.registerLoadMorePost = function () {
    $("#btnLoadMore").click(function () {
        Gauvl.indexPage.showLoadMore = false;
        $("#btnLoadMoreContainer").css("height", "42px");
        $(this).fadeOut(200);
        if (Gauvl.indexPage.initPage != "unread") Gauvl.indexPage.loadMorePost(Gauvl.indexPage.page);
        else Gauvl.Unread.loadSinglePage(false)
    })
};

Gauvl.indexPage.registerModToolsClick = function () {
            ...
            if (a == "OK") {
                $("#entry-" + f).fadeTo('250', 0.0, function () {
                    $(this).slideUp("250", function () {
                        if (Gauvl.indexPage.theme == 2) {
                            $(this).css("margin-bottom", "0px").css("margin-top", "0px");
                            $(this).remove();
                            Gauvl.indexPage.reLayout()
                        } else {
                            $(this).remove()
                        } if ($("[id|=entry]").length == 1) {
                            Gauvl.indexPage.loadMorePost()
                        }
                    })
                })
            }
...
Gauvl.indexPage.registerTopUsersClick = function () {
    $(".topUsers .sort a").click(function (a) {
        a.preventDefault();
        var b = $(this).attr("data-sort");
        if (b == Gauvl.indexPage.sort) return;
        Gauvl.indexPage.sort = b;
        $(".topUsers .sort a").removeClass('selected');
        $(this).addClass('selected');
        $(".topUsers .content").load(baseURL + "top/like/" + b + "?ajax=true")
    })
};
Gauvl.indexPage.loadMorePost = function (c, d) {
    if (Gauvl.indexPage.initPage != "unread") {
        if ($(window).scrollTop() + $(window).height() >= ($(document).height() - 300)) {
            Gauvl.indexPage.showLoadMore = false;
            if (Gauvl.indexPage.isLoading || Gauvl.indexPage.hasNoMore) return;
            Gauvl.indexPage.isLoading = true;
            $('#load_image').show();
            $.get(baseURL + "ajax/loadmore.php?page=" + (c + 1) + "&type=" + Gauvl.indexPage.initPage + "&theme=" + Gauvl.indexPage.theme, function (a) {
                if (a == "NoMorePostToShow") {
                    $("#btnLoadMoreContainer").css("height", "");
                    Gauvl.indexPage.hasNoMore = true;
                    Gauvl.indexPage.showLoadMore = false;
                    $('#load_image').hide();
                    $("#noMorePost").show()
                } else {
                    ...
                    if (Gauvl.indexPage.theme == 1) {
                        $("#listEnd").before(dataToInsert);
                        $('#load_image').hide();
                        Gauvl.indexPage.isLoading = false;
                        Gauvl.indexPage.page++;
                        Gauvl.indexPage.showLoadMore = true;
                        $("#btnLoadMoreContainer").css("height", "");
                    ...
                    }; if (Gauvl.indexPage.page - Gauvl.indexPageNumber == 2) {
                        $("#btnLoadMore").show();
                        Gauvl.indexPage.hasNoMore = true
                    }
                }
...
Gauvl.indexPage.fixVoteGroup = function () {
    ...
    $(".pin-container").each(function () {
        ...
        if ((b.offset().top - d < a + 5)) {
            if (b.attr("data-new") == "true") {
                Gauvl.Unread.addReadPhoto(parseInt(b.attr("data-pid")))
            }
...
Gauvl.indexPage.quickUploadInit = function () {
    var b = $("#postPhotoForm");
    var c = $("#postVideoForm");
    var d = $("#postMusicForm");
    ...
    i.on("input", function () {
        ...
        if (value != "") {
            if (isValidYouTubeURL(value) || isValidVimeoURL(value) || isValidZingTVUrl(value)) {
                ...
                $.getJSON(baseURL + "ajax/mediainfo.php?url=" + encodeURIComponent(value)).done(function (a) {
                    $(".videoInfo").show();
    ...
    j.on("input", function () {
        ...
        if (value != "") {
            if (isValidZingMp3URL(value)) {
                $("#videoLoadingImg").show();
                $.getJSON(baseURL + "/ajax/zinginfo.php?url=" + value).done(function (a) {
                    $(".videoInfo").show();
    ...
    function checkAndSubmit() {
        ...
        if (Gauvl.indexPage.quickUploadType == null) {
            displayMessageDialog("#mediaIsRequired");
            return
        }
        if (Gauvl.indexPage.quickUploadType == "photo") {
        ...
        if (Gauvl.indexPage.quickUploadType == "video") {
            if (i.val() == "") {
        ...
        if (Gauvl.indexPage.quickUploadType == "music") {
            if (j.val() == "") {
                displayMessageDialog("#urlEmptyAlert");
                return
            }
    ...
    function setQuickUploadType() {
        Gauvl.indexPage.quickUploadType = null;
        if (b.is(":visible")) {
            ...
            $("#post_type").val("Photo");
            Gauvl.indexPage.quickUploadType = "photo"
            ...
            $("#post_type").val("Video");
            Gauvl.indexPage.quickUploadType = "video"
        } else if (d.is(":visible")) {
            $("#form-post-image").attr("action", baseURL + "submit?t=v");
            $("#photo_file_upload, #photo_post_url").attr("disabled", "disabled");
            $("#music_post_url").removeAttr("disabled");
            $("#post_type").val("Video");
            Gauvl.indexPage.quickUploadType = "music"
        }
    }
};
Gauvl.indexPage.init = function (a, b, c) {
    Gauvl.indexPage.theme = a;
    Gauvl.indexPage.initPage = b;
    Gauvl.indexPage.page = c;
    Gauvl.indexPageNumber = c;
    Gauvl.common.registerVoteClick();
    $(document).ready(function () {
        if (Gauvl.indexPage.theme == 2) {
            Gauvl.indexPage.layout()
        }
        if (Gauvl.indexPage.initPage == "unread") {
            Gauvl.indexPage.registerLoadMorePost()
        }

        Gauvl.indexPage.registerModToolsClick();
        Gauvl.indexPage.registerTopUsersClick();
        Gauvl.indexPage.quickUploadInit();
        $(window).scroll(function () {
            Gauvl.indexPage.loadMorePost(Gauvl.indexPage.page, Gauvl.indexPage.theme);
            if (Gauvl.indexPage.theme == 1) {
                Gauvl.indexPage.fixVoteGroup()
            }
        })
    });
    window.fbAsyncInit = function () {
        FB.Event.subscribe('edge.create', Gauvl.indexPage.recount);
        FB.Event.subscribe('edge.remove', Gauvl.indexPage.recount);
        FB.Event.subscribe('comment.remove', Gauvl.indexPage.getTopComment);
        FB.Event.subscribe('comment.remove', Gauvl.indexPage.recount);
        FB.Event.subscribe('comment.create', Gauvl.indexPage.getTopComment);
        FB.Event.subscribe('comment.create', Gauvl.indexPage.recount)
    }
};
Gauvl.Facebook = {
    appId: null
};
Gauvl.Facebook.init = function (c) {
    Gauvl.Facebook.appId = c;
    (function (d, s, a) {
...
Gauvl.itemDetail.init = function (c, v) {
    Gauvl.itemDetail.itemID = c;
    videoType = v || "none";
    Gauvl.common.registerVoteClick();
    window.fbAsyncInit = function () {
        FB.Event.subscribe('comment.create', Gauvl.itemDetail.recount);
        FB.Event.subscribe('comment.create', Gauvl.itemDetail.getTopComment);
        FB.Event.subscribe('comment.remove', Gauvl.itemDetail.recount);
        FB.Event.subscribe('comment.remove', Gauvl.itemDetail.getTopComment);
        FB.Event.subscribe('edge.create', Gauvl.itemDetail.recount);
        FB.Event.subscribe('edge.remove', Gauvl.itemDetail.recount)
    };
    Gauvl.itemDetail.registerDeleteClick();
    Gauvl.itemDetail.registerReportClick();
    Gauvl.itemDetail.fixWhenScroll();
    Gauvl.itemDetail.registerNavigationKeys();
    Gauvl.itemDetail.displaySuccessMessage()
};
Gauvl.itemDetail.fixWhenScroll = function () {
    var f = $(".contentWrap");
    var g = $(".head").outerHeight();
    var h = function () {
...
Gauvl.itemDetail.recount = function () {
    $.post(baseURL + "ajax/count/?itemID=" + Gauvl.itemDetail.itemID)
};
Gauvl.itemDetail.getTopComment = function () {
    $.post(baseURL + "ajax/count/?do=getTopComment&itemID=" + Gauvl.itemDetail.itemID)
};
Gauvl.itemDetail.displaySuccessMessage = function () {
    $(document).ready(function () {
...
Gauvl.itemDetail.registerDeleteClick = function () {
        ...
        b.dialog({
            ...
            buttons: {
                "Chắc chắn": function () {
                    ...
                    $.ajax({
                        url: baseURL + "ajax/deletepost.php?pid=" + Gauvl.itemDetail.itemID,
                        context: $(this),
                        success: function (a) {
                            if (a == "OK") {
                                $(this).dialog("close");
                                Gauvl.itemDetail.displayDeleteOKDialog()
                            }
                            if (a == "Fail") {
                                $(this).dialog("close");
...
Gauvl.itemDetail.registerReportClick = function () {
            ...
            buttons: [{
                ...
                click: function () {
                    ...
                    $.ajax({
                        ...
                        data: {
                            itemID: Gauvl.itemDetail.itemID,
                            reason: b,
                            reasonDetail: c
                        },
                        context: $(this),
...
Gauvl.itemDetail.displayDeleteOKDialog = function () {
...
Gauvl.itemDetail.registerNavigationKeys = function () {
...
Gauvl.itemDetail.video = {
    postContent: "postContent",
    vmPlayer: "vmPlayer",
    ytPlayer: "ytPlayer",
    clipAnchor: "#clipAnchor"
};
Gauvl.itemDetail.video.youTube = function () {
    var b = $("." + Gauvl.itemDetail.video.postContent);
    var c = $("#" + Gauvl.itemDetail.video.ytPlayer);
    var d = b.height();
    var e = b.width();
    var f = c.height();
    var g = c.width();
    var h = $(Gauvl.itemDetail.video.clipAnchor).offset().top - 45;
    var i;
    window.onYouTubeIframeAPIReady = function () {
        i = new YT.Player(Gauvl.itemDetail.video.ytPlayer, {
            events: {
    ...
    function onPlayerStateChange(a) {
        if (a.data == YT.PlayerState.PLAYING && !j) {
            Gauvl.itemDetail.displayInfoPane = false;
            $("#rightCol").animate({
                'margin-top': '640px'
            }, 500, function () {
                $(".navigation").animate({
        ...
        if (a.data == YT.PlayerState.ENDED) {
            ...
            Gauvl.itemDetail.displayInfoPane = true
        }
    }
};
Gauvl.itemDetail.video.vimeo = function () {
    var c = $("." + Gauvl.itemDetail.video.postContent);
    var d = $("#" + Gauvl.itemDetail.video.vmPlayer);
    var e = c.height();
    var f = c.width();
    var g = d.height();
    var h = d.width();
    var i = $(Gauvl.itemDetail.video.clipAnchor).offset().top - 45;
    var j = d[0],
        player = $f(j);
...
Gauvl.itemDetail.video.init = function (a) {
    if (a == "vimeo") Gauvl.itemDetail.video.vimeo();
    else Gauvl.itemDetail.video.youTube()
};
Gauvl.settings = {};
Gauvl.settings.checkAndSubmit = function () {
    var e = $("[name=fname]");
...
Gauvl.settings.uiSettingInit = function () {
...
Gauvl.settings.toggle = function () {
...
Gauvl.settings.init = function () {
    Gauvl.settings.checkAndSubmit();
    Gauvl.settings.toggle();
    Gauvl.settings.uiSettingInit()
};
Gauvl.userProfile = {};
Gauvl.userProfile.commentNotification = function (a, b, c) {
    $.ajax({
...
Gauvl.userProfile.init = function (a, b) {
    profileID = a;
    userID = b;
    window.fbAsyncInit = function () {
        FB.Event.subscribe('comment.create', function (a) {
            Gauvl.userProfile.commentNotification(userID, profileID, a)
        })
    }
};
Gauvl.Unread = {
...
Gauvl.LocalStorage.isSupported = function () {
    try {
...
Gauvl.Unread.addReadPhoto = function (a) {
    if (!Gauvl.Unread.shouldTrack) return;
    if (!Gauvl.Unread.justRead[a]) {
        Gauvl.Unread.justRead[a] = true
    }
};
Gauvl.Unread.getJustReadPhotoIds = function () {
    var a = "";
    for (var b in Gauvl.Unread.justRead) {
        if (a == "") a += b;
        else a += "," + b
    }
    return a
};
Gauvl.Unread.submit = function (a) {
    var b = [];
    var c = "";
    for (var d in Gauvl.Unread.justRead) {
        if (!Gauvl.Unread.submitted[d]) {
            Gauvl.Unread.submitted[d] = true;
            b.push(d);
            if (c == "") c += d;
            else c += "," + d
        }
    ...
    $.ajax({
        ...
        success: function () {},
        error: function () {
            for (var i in b) delete Gauvl.Unread.submitted[i]
        },
        complete: function () {}
    })
};
Gauvl.Unread.startTracking = function () {
    $(document).ready(function () {
        Gauvl.Unread.shouldTrack = true;
        setInterval(function () {
            Gauvl.Unread.submit(true)
        }, 30000);
        $(window).unload(function () {
            Gauvl.Unread.submit(false)
        })
    })
};
Gauvl.Unread.init = function () {
    $(document).ready(function () {
        Gauvl.Unread.startTracking();
        Gauvl.Unread.loadSinglePage(true);
        $(window).scroll(function () {
            Gauvl.Unread.loadMore()
        })
    })
};
Gauvl.Unread.loadSinglePage = function (e) {
    Gauvl.Unread.showLoadMore = false;
    if (Gauvl.Unread.isLoading || Gauvl.Unread.hasNoMore) return;
    Gauvl.Unread.isLoading = true;
    $('#load_image').show();
    $.post(baseURL + "ajax/unread.php", {
        excludePhotoIds: Gauvl.Unread.getJustReadPhotoIds(),
        isFirstLoad: e
    }, function (a) {
        if (a == "NoMoreUnread") {
            $("#btnLoadMoreContainer").css("height", "");
            Gauvl.Unread.hasNoMore = true;
            Gauvl.Unread.showLoadMore = false;
            $('#load_image').hide();
            $("#noMorePost").show()
        } else {
            ...
            if (Gauvl.indexPage.theme == 1) {
                $("#listEnd").before(c);
                $('#load_image').hide();
                Gauvl.Unread.isLoading = false;
                Gauvl.Unread.page++;
                Gauvl.Unread.showLoadMore = true;
                $("#btnLoadMoreContainer").css("height", "");
...
Gauvl.Unread.loadMore = function () {
    if ($(window).scrollTop() + $(window).height() >= ($(document).height() - 400)) {
        if ((Gauvl.Unread.page % 4) === 0) {
            if (Gauvl.Unread.showLoadMore === true) {
                $("#btnLoadMore").show();
                Gauvl.Unread.showLoadMore = false
            }
        } else {
            Gauvl.Unread.loadSinglePage(false)
        }
    }
};
        ...
        buttons: [{
            ...
            click: function () {
                if (typeof b === "undefined") {
                    var a = document.URL;
                    window.location.href = baseURL + "login" + a.replace(baseURL, "")
                } else {
                    a = b;
                    window.location.href = baseURL + "login" + a
                }
            }
        }, {
            text: "Bỏ qua",
